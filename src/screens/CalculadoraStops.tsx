...
export default function CalculadoraStops() {
  ...
  const [resultado, setResultado] = useState<{ stop: number; take: number; datosGrafico: any[] } | null>(null);
  ...
  const calcular = () => {
    ...
    // Simulación de velas japonesas
    const datosGrafico = [
      { name: "Vela 1", open: precio, close: precio * 0.99, high: precio * 1.01, low: precio * 0.98 },
      { name: "Vela 2", open: precio * 0.99, close: precio * 1.01, high: precio * 1.02, low: precio * 0.98 },
      { name: "Vela 3", open: precio * 1.01, close: take, high: take * 1.01, low: precio * 1.00 },
      { name: "Vela 4", open: precio, close: stop, high: precio * 1.01, low: stop * 0.98 },
    ];
      ...
      <div className="max-w-3xl mx-auto bg-gray-900 p-6 rounded-2xl shadow-xl border border-gray-700">
        ...
        {resultado && (
          <div className="mt-6 bg-gray-800 p-4 rounded border border-gray-600">
            ...
            <p className="text-green-400 mb-2">🔻 Stop Loss: <strong>{resultado.stop}</strong></p>
            <p className="text-yellow-300 mb-4">🟢 Take Profit: <strong>{resultado.take}</strong></p>
            ...
            <div className="h-64 bg-gray-900 rounded p-2">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={resultado.datosGrafico}>
                  ...
                  <YAxis stroke="#ccc" domain={[resultado.stop * 0.98, resultado.take * 1.02]} />
                  ...
                  <ErrorBar
                    ...
                    width={8}
                    strokeWidth={2}
                    data={resultado.datosGrafico.map((d) => ({ x: d.name, y: d.high, yBottom: d.low }))}
                    stroke="#facc15"
                    direction="y"
                  />
                </ComposedChart>
            ...
            <div className="mt-4 text-sm text-gray-300">
              <p>Con una relación 1:{relacion}, estarías arriesgando un <strong>{riesgoPct}%</strong> para ganar <strong>{+relacion * +riesgoPct}%</strong>.</p>
              <p className="mt-1">Esto implica que, ganando 1 de cada {+relacion + 1} operaciones, ya estarías en break even.</p>
            </div>
          </div>
        )}
      </div>
